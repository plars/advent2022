...
// bad global, but didn't realize I needed it until very late
var threshold int = 1
...
func (m *monkey) inspect(monkeys []monkey) {
	for len(m.items) > 0 {
		...
		// cap the value safely by mod with all test values multiplied together
		// this keeps the numbers from growing out of control
		m.items[0] = m.items[0] % threshold
...
func buildOperation(operation string) func(int) int {
	...
	if line[0] == "*" {
		return func(x int) int { return (x * num) }
	}
	// otherwise it should be +
	return func(x int) int { return (x + num) }
}
